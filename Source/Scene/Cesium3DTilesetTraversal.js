    ...
    function executeBaseTraversal(tileset, root, maximumScreenSpaceError, frameState) {
        ...
        while (stack.length > 0) {
            ...
            if (traverse) {
                for (var i = 0; i < childrenLength; ++i) {
                    var child = children[i];
                    var visible = isVisible(tileset, child, maximumScreenSpaceError, frameState);
            ...
            // TODO : prefetch stuff? Or always block on parent? Prefetch is good as long as it can still refine.
            // TODO : don't want to step on the toes of skip lods.
            if (refines) {
                for (var i = 0; i < childrenLength; ++i) {
                    var child = children[i];
                    var visible = isVisible(tileset, child, maximumScreenSpaceError, frameState);
